...
class _RdConv(EquivariantModule, ABC):
    """
    _RdConv that supports implicit kernels.

    See https://quva-lab.github.io/escnn/_modules/escnn/nn/modules/conv/rd_convolution.html#_RdConv for native implementation.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            use_implicit (bool, optional): whether to use an implicit kernel or not. Default: ``False``
    ...
    def expand_bias(self) -> torch.Tensor:
        ...
        Expand the bias in terms of :class:`~escnn.nn._RdConv.bias`.
    ...
    def expand_filter(self) -> torch.Tensor:
        r"""

        Expand the filter in terms of the :attr:`~escnn.nn._RdConv.weights`.

        Returns:
            the expanded filter

        """
        return self.basisexpansion(self.weights).reshape(
            self.out_type.size, self.in_type.size, *(self.kernel_size,) * self.d
        )
    ...
    def train(self, mode=True):
        ...
        if mode:
            ...
            _filter, _bias = self.expand_filter(), self.expand_bias()
