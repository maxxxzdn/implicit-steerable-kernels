...
__all__ = ["QuotientFourierPointwise", "QuotientFourierPReLU"]
...
class QuotientFourierPointwise(EquivariantModule):
    ...
    def __init__(self,
        ...
        elif function == 'p_prelu':
            if inplace:
                raise ValueError('PReLU does not support inplace operation!')
            else:
...
class QuotientFourierPReLU(QuotientFourierPointwise):
    ...
    def __init__(self,
        ...
        Applies a Inverse Fourier Transform to sample the input features on a quotient space, apply parameterized ReLU 
        point-wise in the spatial domain (Dirac-delta basis) and, finally, computes the Fourier Transform to obtain irreps
        ...
        super(QuotientFourierPReLU, self).__init__(gspace, subgroup_id, channels, irreps, *grid_args, function='p_prelu', inplace=inplace, grid=grid, **grid_kwargs)
