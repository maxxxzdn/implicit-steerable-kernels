...
"""
See https://github.com/RobDHess/Steerable-E3-GNN/blob/main/nbody/dataset_nbody.py for the original code
"""

class NBodyDataset():
    def __init__(self, partition='train', max_samples=1e8, dataset_name="nbody_small", z_component=None):
        """
        Dataset for the N-body problem.
        Args:
            partition: train, val or test
            max_samples: maximum number of samples to load
            dataset_name: nbody, nbody_small or nbody_small_out_dist
            z_component: strength of the z-component (e.g. stiffness of the XY springs)
        """
    ...
    def load(self):
        ...
        path = 'data_nbody'
        loc = np.load(osp.join(dir, path, 'loc_' + self.suffix + '.npy'))
        vel = np.load(osp.join(dir, path, 'vel_' + self.suffix + '.npy'))
        edges = np.load(osp.join(dir, path, 'edges_' + self.suffix + '.npy'))
        charges = np.load(osp.join(dir, path, 'charges_' + self.suffix + '.npy'))
